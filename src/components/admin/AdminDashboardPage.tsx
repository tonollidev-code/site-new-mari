...
export const AdminDashboardPage: React.FC = () => {
  ...
  return (
    <div className="relative min-h-screen bg-[#FCFAF7] text-[#523626] flex flex-col font-sans selection:bg-[#E5C158]/30">
      
      {/* Background Animated Golden Logo */}
      <GlassLogoBackground />

      {/* Top Admin Navigation Bar */}
      <header className="relative z-30 bg-white/85 backdrop-blur-xl border-b border-[#E5C158]/40 sticky top-0 shadow-xs">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          
          {/* Logo & Title */}
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-2xl bg-[#523626] text-[#E5C158] flex items-center justify-center font-serif font-bold text-xl shadow-md border-2 border-[#E5C158]/50">
              M
            </div>
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="bg-[#FAF6F0] text-[#8C6E5D] border border-[#E5C158]/50 text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full flex items-center gap-1 shadow-xs">
                  <ShieldCheck className="w-3 h-3 text-[#C5A059]" />
                  Painel Admin
                </span>
              </div>
              <p className="text-[11px] text-[#8C6E5D] font-medium hidden sm:block">
                Sessão autorizada: <span className="font-mono text-[#523626] font-bold">{currentUser?.email}</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => navigate('/')}
              className="text-xs font-bold text-[#8C6E5D] hover:text-[#523626] bg-white/90 hover:bg-[#F8F5F2] border border-[#EADEDA] hover:border-[#E5C158] px-4 py-2 rounded-full transition-all flex items-center gap-1.5 shadow-xs cursor-pointer"
            ...
            <button
              onClick={() => logoutAdmin()}
              className="bg-rose-50/90 hover:bg-rose-100 text-rose-700 border border-rose-200 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 transition-colors shadow-xs cursor-pointer"
            >
        ...
        {/* Tab Navigation Menu */}
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center gap-2 overflow-x-auto border-t border-[#EADEDA]/60 py-2.5 scrollbar-none">
          {[
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-2xl text-xs font-bold transition-all whitespace-nowrap flex items-center gap-2 cursor-pointer ${
                  isActive
                    ? 'bg-[#523626] text-[#F5E5C9] shadow-md border border-[#E5C158]/50 ring-1 ring-[#E5C158]/20'
                    : 'bg-white/80 hover:bg-white text-[#7D5E4D] hover:text-[#523626] border border-[#EADEDA]/80 backdrop-blur-xs'
                }`}
              >
                <Icon className={`w-3.5 h-3.5 ${isActive ? 'text-[#E5C158]' : 'text-[#C5A059]'}`} />
                <span>{tab.label}</span>
                {tab.badge !== null && tab.badge !== undefined && (
                  <span className="bg-[#E5C158] text-[#3D2314] text-[10px] font-extrabold px-1.5 py-0.2 rounded-full shadow-xs">
                    {tab.badge}
                  </span>
                )}
              </button>
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-8 animate-in fade-in">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-5">
              ...
              <div className="bg-white/90 backdrop-blur-md p-5 rounded-3xl border border-[#E5C158]/35 shadow-[0_8px_20px_rgba(44,24,16,0.03)] hover:shadow-md transition-shadow">
              ...
              <div className="bg-[#523626] text-[#F5E5C9] p-5 rounded-3xl border border-[#E5C158]/50 shadow-lg shadow-[#523626]/20 col-span-2 md:col-span-1">
            ...
            <div className="bg-white/90 backdrop-blur-md p-6 sm:p-8 rounded-3xl border border-[#E5C158]/40 shadow-[0_10px_30px_rgba(44,24,16,0.04)] space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-xs font-bold text-[#C5A059] hover:text-[#523626] flex items-center gap-1 hover:underline cursor-pointer"
                >
              ...
              {upcomingBookings.length === 0 ? (
                <div className="p-8 text-center text-xs text-[#8C6E5D] italic bg-[#F8F5F2]/80 backdrop-blur-xs rounded-2xl border border-[#EADEDA]">
                ...
                <div className="space-y-3">
                  {upcomingBookings.map((b) => (
                    <div
                      key={b.id}
                      className="p-4 rounded-2xl bg-[#F8F5F2]/80 hover:bg-[#F8F5F2] transition-colors border border-[#EADEDA] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs"
                    >
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span className="font-bold text-[#523626] text-sm">{b.clientName}</span>
                          <span className="text-[10px] bg-white px-2 py-0.5 rounded-md border border-[#E5C158]/30 text-[#8C6E5D] font-mono">
                            #{b.id}
                          </span>
                        </div>
                      ...
                      <div className="flex items-center gap-2 self-end sm:self-center">
                        ...
                        <button
                          onClick={() => setActiveTab('bookings')}
                          className="bg-white border border-[#EADEDA] hover:border-[#E5C158] px-3 py-1 rounded-full text-[11px] font-bold text-[#523626] shadow-xs cursor-pointer"
                        >
                          Gerenciar
                        </button>
                      </div>
