...
import { Calendar, Clock, User, Phone, MessageCircle, Check, X, Ban, CheckCircle2, Search, Filter } from 'lucide-react';
...
export const BookingsManager: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-5 rounded-3xl border border-[#E5C158]/40 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
        {/* Status Tabs */}
          ...
          {[
            ...
            <button
              ...
              className={`px-3.5 py-2 rounded-2xl text-xs font-bold transition-all whitespace-nowrap flex items-center gap-2 ${
                filterStatus === tab.id
                  ? 'bg-[#523626] text-[#F5E5C9] shadow-sm'
                  : 'bg-[#F8F5F2] text-[#7D5E4D] hover:bg-[#EADEDA]'
              }`}
            >
              <span>{tab.label}</span>
        ...
        <div className="relative w-full md:w-64">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 rounded-2xl border border-[#EADEDA] text-xs focus:outline-none focus:border-[#E5C158] text-[#523626]"
          />
        </div>
      </div>
      ...
      {filteredBookings.length === 0 ? (
        <div className="bg-white rounded-3xl border border-[#EADEDA] p-12 text-center max-w-md mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredBookings.map((b) => (
            <div
              key={b.id}
              className="bg-white rounded-3xl border border-[#E5C158]/30 shadow-md p-6 flex flex-col justify-between hover:shadow-lg transition-shadow relative overflow-hidden"
            >
              <div>
                {/* Header info */}
                <div className="flex items-center justify-between gap-2 mb-4">
                  <span className="text-[10px] font-bold text-[#C5A059] uppercase tracking-wider bg-[#F8F5F2] px-2.5 py-1 rounded-full border border-[#E5C158]/20">
                    #{b.id}
                  </span>
                ...
                {/* Date & Time */}
                <div className="bg-[#F8F5F2] p-3.5 rounded-2xl border border-[#EADEDA] mb-4 space-y-1.5 text-xs text-[#523626]">
                ...
                <div className="space-y-1.5 text-xs text-[#523626] mb-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <a
                      ...
                      className="text-emerald-600 hover:text-emerald-700 font-bold flex items-center gap-1 text-[11px] bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200"
                    >
                  ...
                  {b.notes && (
                    <p className="text-[11px] text-[#8C6E5D] bg-gray-50 p-2 rounded-xl mt-2 italic">
                      Obs: "{b.notes}"
                    </p>
                  )}
                </div>
              ...
              <div className="pt-4 border-t border-[#EADEDA]/60 grid grid-cols-2 gap-2">
                {b.status === 'pending' && (
                  <>
                    <button
                      ...
                      className="bg-emerald-600 hover:bg-emerald-700 text-white py-2 rounded-xl text-xs font-bold transition-all shadow-sm flex items-center justify-center gap-1"
                    >
                      <Check className="w-3.5 h-3.5" />
                      <span>ACEITAR</span>
                    </button>
                    <button
                      ...
                      className="bg-rose-600 hover:bg-rose-700 text-white py-2 rounded-xl text-xs font-bold transition-all shadow-sm flex items-center justify-center gap-1"
                    >
                      <X className="w-3.5 h-3.5" />
                      <span>RECUSAR</span>
                    </button>
                ...
                {b.status === 'confirmed' && (
                  <>
                    <button
                      ...
                      className="bg-purple-600 hover:bg-purple-700 text-white py-2 rounded-xl text-xs font-bold transition-all shadow-sm flex items-center justify-center gap-1"
                    >
                      <CheckCircle2 className="w-3.5 h-3.5" />
                      <span>CONCLUIR</span>
                    </button>
                    <button
                      ...
                      className="bg-gray-600 hover:bg-gray-700 text-white py-2 rounded-xl text-xs font-bold transition-all shadow-sm flex items-center justify-center gap-1"
                    >
                      <Ban className="w-3.5 h-3.5" />
                      <span>CANCELAR</span>
                    </button>
                ...
                {(b.status === 'recused' || b.status === 'cancelled' || b.status === 'completed') && (
                  <button
                    ...
                    className="col-span-2 bg-[#523626] hover:bg-[#3D2314] text-[#F5E5C9] py-2 rounded-xl text-xs font-bold transition-all shadow-sm"
                  >
                    Alterar Status
                  </button>
                )}
          ...
          <div className="bg-white rounded-3xl border border-[#E5C158] p-6 max-w-sm w-full shadow-2xl text-center">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex-1 py-2.5 rounded-full border border-[#EADEDA] text-xs font-bold text-[#7D5E4D] hover:bg-gray-50"
              >
                Voltar
              </button>
              <button
                onClick={executeStatusChange}
                className="flex-1 py-2.5 rounded-full bg-[#523626] text-[#F5E5C9] text-xs font-bold hover:bg-[#3D2314] shadow-md"
              >
                Confirmar
              </button>
            </div>
